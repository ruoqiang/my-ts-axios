...
import { getAjaxRequest } from '../test/helper'
...
describe('interceptors', () => {
  ...
  test('should add a response interceptor', done => {
    let resopnse: AxiosResponse
    const instance = axios.create()
    instance.interceptors.response.use(data => {
      data.data = data.data + ' - added by interceptor'
      return data
    })
    ...
    getAjaxRequest().then(request => {
      ...
      setTimeout(() => {
        expect(resopnse.data).toBe('OK - added by interceptor')
        done()
      }, 100)
    })
  })
