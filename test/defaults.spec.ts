...
describe('defaults', () => {
  ...
  test('should be used by coustom instance if set before instance created', () => {
    ...
    return getAjaxRequest().then(request => {
      ...
      delete axios.defaults.baseURL // 这里记得删除，不然会影响后面的test case
    })
  })
  test('should not be used by coustom instance if set after instance created', () => {
    const instance = axios.create()
    axios.defaults.baseURL = 'http://example.org/'
    instance.get('/foo')
